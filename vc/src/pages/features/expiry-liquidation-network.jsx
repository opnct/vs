...
export default function FeatureExpiryLiquidationNetwork() {
  const [logs, setLogs] = useState([]);
  const [metrics, setMetrics] = useState({ efficiency: 0, latency: 0, load: 0 });

  useEffect(() => {
    const timer = setInterval(() => {
      setLogs(prev => [{ id: Date.now(), timestamp: new Date().toLocaleTimeString(), message: 'Processing Expiry Liquidation data shard...', status: 'SUCCESS' }, ...prev].slice(0, 5));
    ...
    <div className="min-h-screen bg-[#0a0a0a] text-white font-sans animate-in fade-in duration-700">
      <section className="relative h-[70vh] flex items-center px-6 md:px-24 border-b border-[#222]">
        ...
        <div className="relative z-20 max-w-4xl">
          ...
          <h1 className="text-6xl md:text-8xl font-black tracking-tighter leading-[0.9] mb-8 uppercase italic italic">Expiry Liquidation</h1>
          <p className="text-xl md:text-2xl text-gray-400 max-w-2xl font-medium leading-relaxed border-l-2 border-[#005ea2] pl-6">Connect to local businesses to sell near-expiry goods. Powered by VyaparSetu AI.</p>
        </div>
      </section>
      ...
      <section className="py-24 px-6 md:px-24 bg-[#0a0a0a]">
        <div className="grid lg:grid-cols-12 gap-12 max-w-7xl mx-auto">
          ...
          <div className="lg:col-span-4 bg-[#005ea2] p-8 rounded-sm shadow-xl relative overflow-hidden group">
            ...
            <p className="text-sm text-blue-100 mb-8 leading-relaxed">Deploying Expiry Liquidation provides enterprise-level insight into your monthly retail savings and inventory movement.</p>
      ...
      <section className="py-24 px-6 md:px-24 bg-white text-black">
        <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-24 items-center">
          <div>
            ...
            <p className="text-xl leading-relaxed text-gray-700 mb-10 font-medium">VyaparSetu leverages a proprietary Stateless Ledger Event Bus to ensure that every Expiry Liquidation operation is encrypted at the source using AES-256 and never persisted on unencrypted nodes.</p>
          </div>
