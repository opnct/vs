...
export default function Ledgers() {
  ...
  const [log, setLog] = useState('Line 1: Loading ledgers...');
  ...
  const handleCreate = async () => {
    ...
    if (!window.__TAURI_INTERNALS__) return setLog('ERR: Cannot save. Native engine missing.');
      ...
      <div>
        ...
        <div className="flex gap-4 items-end">
          <div className="flex-1">
            <label className="text-xs text-np-muted block">Name</label>
            <input ref={nameRef} type="text" value={form.name} onChange={e=>setForm({...form, name: e.target.value})} className="w-full" />
          </div>
          <div className="w-64">
            <label className="text-xs text-np-muted block">Under Group</label>
          ...
          <div className="w-32">
            <label className="text-xs text-np-muted block">Open Bal</label>
            <input type="number" value={form.bal} onChange={e=>setForm({...form, bal: e.target.value})} className="w-full" />
          </div>
